...
And the answer will be $(0+1(0+1))((0+1)(0+1))^* $
...
For all the string in the \textbf{\textit{perfect shuffle}} set, it's obvious that the NFA
can accept it.\\
$\forall s:string$ that the NFA can accept, because the NFA always switches between 
states like $(\_,\_,A)$ and $(\_,\_,B)$, $s$ must be written as $a_1b_1a_2b_2...$, such that
after accepting $a_i$, the state should be $(\_,\_,B)$, and after accepting $b_i$, the
state should be $(\_,\_,A)$. Since every state like $(\_,\_,A)$ would apply a transition on the 
first part of the state, so $\delta_A(q_A,a_1a_2a_3...) \in F_A$, that's to say, $a_1a_2a_3... \in L(A)$.
Similarly, $b_1b_2b_3... \in L(B)$
...
\[\Sigma = \Sigma \]
For all the string in the \textbf{\textit{shuffle}} set, it's obvious that the NFA
can accept it.\\
$\forall s:string$ that the NFA can accept, after accepting each character, the NFA
would transit either state A or state B but not both. So there must
 be one sub-sequence which can be accept by A and the left sub-sequence can be 
 accepted by B.
